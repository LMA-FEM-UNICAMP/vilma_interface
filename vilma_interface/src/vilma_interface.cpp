  ...
  switch (type_tx)
  {
    ...
    case TxTypeMA::SENSORS_MA: {
      mutex_vilma_sensors_.lock();
      {
        ...
        if (get_operation_state(static_cast<int>(sensors_ma_msg_.data[SensorsMA::OPERATION_STATE])) ==
            RXOperationModeMA::INITIAL_STATE_MODE)
        {
          set_control_mode(AutowareControlMode::MANUAL);

          auto msg = std_msgs::msg::String();
          msg.data = "MA is initializing...";
          hmi_status_pub_->publish(msg);
        }

        if (sensors_ma_msg_.data[SensorsMA::TIME_PCC_BRAKE] == -1.0)
        {
          if (AutowareControlMode::AUTONOMOUS || AutowareControlMode::AUTONOMOUS_VELOCITY_ONLY)
          {
        ...
        if (lost_ecu_connection_.trig(sensors_ma_msg_.data[SensorsMA::TIME_PCC_BRAKE] == -1.0))
        {
          ...
          RCLCPP_FATAL(this->get_logger(),
                       "Please restart the vehicle! Connection with brake ECU lost for more then 5 seconds.");
          auto msg = std_msgs::msg::String();
        ...
        //* Verify that the user brake pedal was pressed | Emergency -- User braking
        {
                  ...
                  !steer_only_mode_))
          {
            ...
            auto msg = std_msgs::msg::String();
            msg.data = "Control Mode changed to MANUAL by user command";
            hmi_status_pub_->publish(msg);
          }
          else
          {
        ...
        double steering_tire_angle_rad = sensors_ma_msg_.data[SensorsMA::STEER_POS] / 19.2895184102108;
  ...
  if (change_control_mode_enabled_.load())  /// If yes
  {
    ...
    switch (control_mode)
    {
      ...
      case JoystickMA::MODE_STEER_COMMAND_LOOK_ZERO:
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 500,
                             "Control Mode changed to SEEK STEER-ZERO MODE");
...
void VilmaInterface::ma_timer_callback()
{
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Communication error with MA!");
...
void VilmaInterface::control_timer_callback()
{
  ...
  mutex_joystick_command_.lock();
  {
    if (joystick_command_[JoystickMA::GAS_COMMAND] == JoystickMA::GAS_COMMAND_POSITION)
    {
      ...
      mutex_velocity_controller_.lock();
      {
        velocity_controller_.calculate(control_action, this->get_clock()->now().nanoseconds());
      }
      mutex_velocity_controller_.unlock();
    }
