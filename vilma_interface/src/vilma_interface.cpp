...
VilmaInterface::VilmaInterface() : Node("vilma_interface")
{
  ...
  brake_user_pressure_set_emergency_ =
    this->get_parameter("brake_user_pressure_set_emergency").as_double();
  ...
  autoware_command_time_validity_ms_ =
    this->get_parameter("autoware_command_time_validity_ms").as_int();
  communication_timeout_ms_ = this->get_parameter("communication_timeout_ms").as_int();
  double joystick_command_time_validity_ms =
    this->get_parameter("joystick_command_time_validity_ms").as_int();
  ...
  bool udp_configure_response = ma_udp_client.configure(
    pc_udp_port, ma_udp_port, ma_ip, boost::posix_time::millisec(udp_timeout_ms), to_ma_length_,
    from_ma_length_);
  ...
  ma_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(ma_timer_period_ms_),
    std::bind(&VilmaInterface::ma_timer_callback, this), timers_callback_group_);

  ma_sleep_timer_ = this->create_wall_timer(
    std::chrono::minutes(ma_sleep_period_min_), std::bind(&VilmaInterface::ma_sleep_callback, this),
    timers_callback_group_);

  control_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(control_timer_period_ms_),
  ...
  gear_cmd_sub_ = this->create_subscription<autoware_vehicle_msgs::msg::GearCommand>(
    ...
    std::bind(&VilmaInterface::gear_cmd_callback, this, _1), autoware_sub_options);

  engage_sub_ = this->create_subscription<autoware_vehicle_msgs::msg::Engage>(
    "/vehicle/engage", rclcpp::QoS{1}, std::bind(&VilmaInterface::engage_callback, this, _1),
    autoware_sub_options);

  control_mode_request_server_ =
  ...
  joystick_ma_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
    ...
    std::bind(&VilmaInterface::joystick_ma_callback, this, _1), debug_sub_options);
  ...
  hmi_throttle_pub_ =
    this->create_publisher<std_msgs::msg::Float32>("/hmi/throttle", rclcpp::QoS{1});
...
unsigned short VilmaInterface::to_ma()
{
  ...
  if (to_ma_vector_[0] == 0.0)  /// If not new, just request MA data
  {
    ...
    std::fill(
      to_ma_vector_.begin() + 3, to_ma_vector_.end(),
  ...
  } else  /// Else, request data and send Joystick command
  {
    ...
    ma_operation_mode_ =
      OperationModeMA::JOYSTICK_MODE;  /// Set MA's operation mode to joystick indefinitely
  ...
  switch (type_tx) {
    ...
    case TxTypeMA::SENSORS_MA: {
      ...
      mutex_vilma_sensors_.lock();
      {
        ...
        if (
          get_operation_state(static_cast<int>(sensors_ma_msg_.data[SensorsMA::OPERATION_STATE])) ==
          RXOperationModeMA::INITIAL_STATE_MODE) {
        ...
        {
          if (
            user_command_handler_.trig(
              ((sensors_ma_msg_.data[SensorsMA::BRAKE_USER_PRESSURE] >=
                brake_user_pressure_set_emergency_) ||
        ...
        double steering_tire_angle_rad =
          sensors_ma_msg_.data[SensorsMA::STEER_POS] / 19.2895184102108;

        if (abs(steering_tire_angle_rad) <= max_steering_tire_angle_rad_) {
          ...
          steering_report_msg.steering_tire_angle = steering_tire_angle_rad;
      ...
      mutex_vilma_state_.lock();
      {
        ...
        //* Publish velocity status topic to Autoware
        {
          ...
          velocity_report_msg.longitudinal_velocity =
            state_ma_msg_.data[StateMA::LONGITUDINAL_SPEED];
      ...
      switch (control_mode) {
        ...
        case AutowareControlMode::MANUAL:
          ...
          RCLCPP_INFO_THROTTLE(
            this->get_logger(), *this->get_clock(), 500, "Control Mode changed to MANUAL");
        ...
        case AutowareControlMode::AUTONOMOUS:
          ...
          if (!steer_only_mode_) {
            ...
            mutex_vilma_state_.lock();
            {
              joystick_command_[JoystickMA::STEER_VALUE] =
                get_steering_value(vilma_steer_tire_angle_.load());
            }
            mutex_vilma_state_.unlock();
          }
          mutex_joystick_command_.unlock();  /// Unlock mutex
          ...
          RCLCPP_INFO_THROTTLE(
            this->get_logger(), *this->get_clock(), 500, "Control Mode changed to AUTONOMOUS");
        ...
        case AutowareControlMode::AUTONOMOUS_STEER_ONLY:
          ...
          RCLCPP_INFO_THROTTLE(
            ...
            "Control Mode changed to AUTONOMOUS_STEER_ONLY");
        ...
        case AutowareControlMode::AUTONOMOUS_VELOCITY_ONLY:
          ...
          mutex_velocity_controller_.lock();
          {
            velocity_controller_.reset(this->get_clock()->now().nanoseconds());
          }
          mutex_velocity_controller_.unlock();
          ...
          RCLCPP_INFO_THROTTLE(
            this->get_logger(), *this->get_clock(), 500,
            "Control Mode changed to AUTONOMOUS_VELOCITY_ONLY");
        ...
        case JoystickMA::MODE_STEER_COMMAND_LOOK_ZERO:
          ...
          mutex_joystick_command_.lock();
          {
            ...
            //* Set steer command in calibrate zero mode
            joystick_command_[JoystickMA::STEER_COMMAND] = JoystickMA::STEER_COMMAND_LOOK_ZERO;
...
void VilmaInterface::ma_timer_callback()
{
  ...
  if (udp_request_output.empty())  /// Successful request (no errors reported)
  {
    ...
    mutex_joystick_command_.lock();  /// Lock mutex to update shared variable joystick_command_
    {
      ...
      // again.
      joystick_command_[0] =
        (joystick_command_[0] == to_ma_vector_[0]) ? 0.0 : joystick_command_[0];
    }
    mutex_joystick_command_.unlock();  /// Unlock mutex

    //* Logging one time per second the MA period and delay.
    RCLCPP_INFO_THROTTLE(
      this->get_logger(), *this->get_clock(), 1000, "MA period: %f | MA delay: %f",
  ...
  } else  /// Error occurred
  {
    ...
    else if (ma_udp_client.get_udp_error() > 1000)  /// Sleep UDP for some time and reconnect
    {
      ...
      RCLCPP_WARN(
        this->get_logger(), "Suspending MA communication. Waiting %d minutes to reconnect to MA...",
  ...
  RCLCPP_INFO_THROTTLE(
    this->get_logger(), *this->get_clock(), 1000, "MA timer loop time: %f ms",
...
void VilmaInterface::control_timer_callback()
{
    ...
    .lock();
  {
    if (joystick_command_[JoystickMA::GAS_COMMAND] == JoystickMA::GAS_COMMAND_POSITION) {
      ...
      mutex_velocity_controller_.lock();
      {
        velocity_controller_.calculate(
          control_action, vilma_longitudinal_speed_.load(), this->get_clock()->now().nanoseconds());
      }
      mutex_velocity_controller_.unlock();
    }
...
/**
 *
 * @brief Receive the control reference for lateral and longitudinal control and process the
 * steering, gas and brake values, applying low-level controller to longitudinal velocity.
 * @param msg Ackermann control topic message
 * @return void
 */
void VilmaInterface::control_cmd_callback(
  const autoware_control_msgs::msg::Control::ConstSharedPtr msg)
{
  ...
  mutex_joystick_command_.lock();  /// Lock mutex to update shared variable joystick_command_
  {
    ...
    joystick_command_[JoystickMA::STEER_VALUE] =
      get_steering_value(msg->lateral.steering_tire_angle);
  }
  mutex_joystick_command_.unlock();  /// Unlock mutex
}
 ...
 * @return void
 */
void VilmaInterface::gear_cmd_callback(
 ...
 * @return void
 */
void VilmaInterface::joystick_ma_callback(
